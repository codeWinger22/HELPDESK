...
def get_user(username):
    ...
    l = list(user_data['_id'], user_data['email'], user_data['password']) if user_data else None
    return l
...
def save_room(room_name, created_by):
    ...
    print(room_id,room_name,created_by)
    return room_id
...
def get_room_existence(username):
    d = dict()
    count = len(list(room_members_collection.find({'_id.username': username})))
    a = room_members_collection.find({'_id.username': username})
    print(a , "from db")
...
def add_room_member(room_id, room_name, username, added_by, is_room_admin=False):
    ...
    print("successfully added room members")
